  ...
  const loadUserFromDatabase = async (userId: string) => {
    try {
      ...
      if (!error && data) {
        ...
        // Se usuário não existe, usar dados do auth
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message?.includes('already registered') || error.message?.includes('already exists') || error.message?.includes('User already registered')) {
          ...
          throw new Error('Erro de conexão com Supabase. Verifique sua internet.');
        } else {
      ...
      if (data?.user) {
        ...
        // Aguardar um pouco para o trigger criar o usuário na tabela public.users
      ...
      // Re-throw para que o componente possa capturar a mensagem
      if (error.message) {
        throw error;
      }
